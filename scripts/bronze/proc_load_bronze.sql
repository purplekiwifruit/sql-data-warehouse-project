...
BEGIN
	...
	BEGIN TRY
		...
		WITH (
			FIRSTROW = 2, --skip the first row in the file
			FIELDTERMINATOR = ',',  -- speficy the delimiter used by the file
			TABLOCK --lock the entir table during loading, enhance performance
		);
		...
		PRINT '>>Inserting Date into: .erp_cust_az12';
		...
		PRINT '>> Batch Loading Completed. Batch Load Duration: ' + CAST(DATEDIFF(second, @batch_start_time, @batch_end_time) AS NVARCHAR) + ' seconds';
	END TRY
	BEGIN CATCH
		PRINT '===============================================================';
